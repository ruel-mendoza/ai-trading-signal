...
6. **Non-Forex Trend Following (trend_non_forex)**: SPX, NDX, XAU/USD, XAG/USD, WTI/USD, BTC/USD, ETH/USD. D1 candles, evaluates at 4:00 PM ET. Uses v4 advance endpoint for real-time price (entry/exit decisions), historical candles for indicator computation (SMA, ATR). LONG only — advance close > highest close of last 50 days AND SMA(50) > SMA(100). No short entries. Signal includes action="ENTRY", direction="LONG". Exit via 3x ATR(100) trailing stop (ATR fixed at entry). Exit check also uses v4 advance price with fallback to cached candles. Signal timestamp uses current ET time. Scheduled via APScheduler at 16:00 US/Eastern. Module: `trading_engine/strategies/trend_non_forex.py`.
