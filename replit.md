...
- **Admin Interface:** A dedicated admin dashboard for monitoring signals, managing API keys, tracking credit usage, and configuring user settings. Includes "Scheduler Health" tab (job logs, 24h stats, watchdog heartbeat, running status) and "System Status" tab (all production hardening features: rate limiting, security headers, error handler, database, watchdog, API key, kill switch, webhooks, auto-restart, misfire recovery with live status badges).
