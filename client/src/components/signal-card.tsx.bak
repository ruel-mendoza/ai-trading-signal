...
export function SignalCard({ signal }: { signal: Signal }) {
  ...
  return (
    ...
    >
      ...
      {signal.exitInstructions && (
        <div className="border-t border-card-border/60">
          <button
            className="w-full flex items-center justify-between px-4 py-2 text-xs text-blue-400 hover:text-blue-300 transition-colors group/exit"
            onClick={(e) => { e.stopPropagation(); setExitOpen((v) => !v); }}
            data-testid={`button-exit-toggle-${signal.id}`}
          >
          ...
          {exitOpen && (
            <div
              ...
              data-testid={`text-exit-instructions-${signal.id}`}
            >
              {signal.exitInstructions}
            </div>
          )}
