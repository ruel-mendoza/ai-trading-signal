...
class ForexTrendFollowingStrategy(BaseStrategy):
    ...
    def evaluate(
        ...
        logger.info(
            f"[TREND-FOREX] {asset} | SHORT check: close < lowest_50d={close_below_lowest} "
        ...
        if open_position_data:
            ...
            if pos_dir == "BUY":
                ...
                if new_lowest < stored_lowest:
                    ...
                    logger.info(
                        f"[TREND-FOREX] {asset} | TROUGH UPDATE #{pos_id} | "
        ...
        if close_above_highest and sma50_above_sma100:
            ...
            if open_position_data and open_position_data.get("direction") == "SELL":
                logger.info(f"[TREND-FOREX] {asset} | IDEMPOTENCY: Existing open SHORT position - skipping")
                return SignalResult()
            ...
            logger.info(f"[TREND-FOREX] {asset} | No entry conditions met - no action")
    ...
    def check_exits(self) -> list[dict]:
        ...
        for pos in positions:
            ...
            logger.info(
                f"[TREND-FOREX-EXIT] Position #{pos_id} | ATR locked at entry: {atr_at_entry:.6f} (read from DB, not recalculated)"
            )
