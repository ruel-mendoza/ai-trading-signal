...
class NonForexTrendFollowingStrategy(BaseStrategy):
    ...
    def evaluate(
        ...
        logger.info(f"[TREND-NONFX] {asset} | 50-day highest={highest_50d:.5f} | 50-day lowest={lowest_50d:.5f}")
        logger.info(
            f"[TREND-NONFX] {asset} | LONG check: close > highest_50d={close_above_highest} "
            f"AND SMA50 > SMA100={sma50_above_sma100}"
        )
        logger.info(
            f"[TREND-NONFX] {asset} | SHORT check: close < lowest_50d={close_below_lowest} "
            f"AND SMA50 < SMA100={not sma50_above_sma100}"
        )
        ...
        if open_position_data:
            ...
            if pos_dir == "BUY":
                ...
                stored_lowest = open_position_data.get("lowest_price_since_entry") or open_position_data["entry_price"]
                new_lowest = min(stored_lowest, current_close)
                if new_lowest < stored_lowest:
                    update_position_tracking(pos_id, lowest_price=new_lowest)
                    logger.info(
                        f"[TREND-NONFX] {asset} | TROUGH UPDATE #{pos_id} | "
                        f"prev_lowest={stored_lowest:.5f} → new_lowest={new_lowest:.5f} (persisted to DB)"
                    )
                if pos_atr is not None:
                    trailing_stop = new_lowest + (pos_atr * TRAILING_STOP_ATR_MULT)
                    logger.info(
                        f"[TREND-NONFX] {asset} | ACTIVE TRADE #{pos_id} | "
                        f"direction=SELL | entry={open_position_data['entry_price']:.5f} | "
                        f"ATR_at_entry={pos_atr:.6f} (FIXED from DB) | "
                        f"lowest_since_entry={new_lowest:.5f} | "
                        f"current_trailing_stop={trailing_stop:.5f}"
                    )
        ...
        if close_above_highest and sma50_above_sma100:
            ...
            logger.info(f"[TREND-NONFX] {asset} | ALL CONDITIONS MET: SHORT")
            logger.info(
                f"[TREND-NONFX] {asset} | ATR({ATR_PERIOD}) at entry = {atr_val:.6f} "
                f"(FIXED for trade lifetime)"
            )
            ...
            logger.info(f"[TREND-NONFX] {asset} | No entry conditions met — no action")
        ...
        for pos in positions:
            ...
            if direction == "BUY":
                ...
                if current_close < trailing_stop:
                    ...
                    logger.info(f"[TREND-NONFX-EXIT] Position #{pos_id} | EXIT: trailing_stop | Trade closed - reversal now permitted")
                    ...
                    logger.info(f"[TREND-NONFX-EXIT] Position #{pos_id} | Holding BUY position")
                ...
                logger.info(
                    f"[TREND-NONFX-EXIT] Position #{pos_id} | SELL tracking | "
                ...
                if current_close > trailing_stop:
                    ...
                    logger.info(f"[TREND-NONFX-EXIT] Position #{pos_id} | Holding SELL position")
